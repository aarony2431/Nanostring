...
##Load HZBio Data
{
  ...
  TestData_Norm <- read.csv(r"{C:\Users\ayu\OneDrive - Avidity Biosciences\Documents\Data\Nanostring\HZBio\DMPK_Pset2\2022.06.21 - HZBio DMPK Pset2 -NormCounts.csv}", row.names = 1, check.names = FALSE)
...
``` {r, echo=FALSE}
{
  logitPredict <- function(test, pred, predictors) {
    ...
    print(ggplot(mydata, aes(as.factor(logit), as.factor(predictor.value)))+
  ...
  }
  {
    ...
    TestData_Predicted <- logitPredict(testlogitSet, pred = input_pred, predictors = input_predictors) %>% clean_data(ctrl_genes = ctrlNames, remove_additional = 'Row.names')
  }
  ...
  if(save) {
    ...
    filename <- r"{SERIALBIOPSY_ContentNorm_PredictionData.csv}"
